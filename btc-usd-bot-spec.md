...
- **New candidate filter (implemented, untested): normalized ATR (NATR = ATR/price) vs. its own trailing history.** A different axis from ADX/SMA200 — those measure trend *strength*/*extension*; this measures volatility *expansion*, on the premise (7c) that BTC moves in volatility clusters and compression often precedes the moves a trend-following sub-strategy is meant to catch. Self-relative (compared against its own trailing-lookback percentile, not a fixed universal number), so it should generalize better across assets/price levels than an absolute threshold would. No backtest results yet — log them here once run.
...
**f) New candidate (implemented, untested): market-structure breakout + retest**
Find the most recent confirmed swing high/low (a fractal — a bar whose high/low is the extreme of several bars on each side of it), wait for a close beyond it (breakout), wait for price to come back and retest that level, then enter on confirmed rejection/continuation. Deliberately a different entry hypothesis from (a)/(e) — those ask "what do the indicators say"; this asks "what is price actually doing." Uses only the most recent confirmed swing point per direction rather than tracking every unresolved one — simpler as a first version, at the cost of possibly missing an older level's still-live cycle if that turns out to matter. No backtest results yet — log them here once run, same as the other candidates.
