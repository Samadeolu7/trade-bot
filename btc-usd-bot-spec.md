...
**c) Volatility breakout: Bollinger Band squeeze + volume confirmation (implemented as `vol_expansion`, shadow-testing)**
...
Implemented as `VolatilityExpansionBreakoutStrategy` (`bot/strategy/vol_expansion.py`), without the volume-confirmation half of the original idea (kept to price/volatility only, matching every other strategy here — volume confirmation remains a candidate future refinement, not yet built). Bollinger Band width, normalized by price, counts as a "squeeze" when it's at or below its own rolling `squeeze_percentile` over `squeeze_lookback` bars — self-relative like `NatrRegimeFilter`, not a fixed universal threshold. A signal only fires when the bar *before* the breakout was squeezed, width is now expanding, and price closes beyond the *prior* bar's band edge (comparing against a same-bar band would let one extreme close pull its own band wide enough to contain itself, hiding real breakouts — same reasoning as donchian's shifted channel). ATR trailing stop, same mechanism as the other trend strategies. Deployed as a fourth concurrent shadow-run candidate (`vol_expansion`, standalone, no regime filter) — no backtest verdict logged yet.

**d) Crypto-specific: funding rate mean-reversion (implemented, shadow-testing)**
...
Implemented as `FundingFilteredStrategy` (`bot/strategy/funding_filter.py`): wraps any base trend strategy (default donchian) and vetoes a new long when funding exceeds `high_threshold` (crowded-long risk) or a new short when funding is below `low_threshold` (crowded-short risk) — a confirmation/veto, not its own entry rule. `high_threshold`/`low_threshold` (default ±0.0005, an untested guess — sweep it) and `base_strategy` are config-driven (`strategy.funding_filtered`). Funding data comes from a separate `binanceusdm` (USD-M futures) client and its own `funding_rates` DB table, since funding is a futures-only concept absent from the spot client used everywhere else — looked up per bar via as-of/backward alignment (funding prints ~every 8h, far sparser than daily candles). Live shadow runs refresh this data every iteration via the new `Strategy.before_poll()` hook; backtests get a fixed funding history for the whole window. Deployed as a fifth concurrent shadow-run candidate (`donchian_funding_filtered`) — no backtest verdict logged yet, being evaluated live per the "testing hypotheses is cheap now" approach (Section 13).
...
## 9.5. Manual-Trading Recommendation Layer (2026-09-09)

A second, independent alert channel — **not** the automated Quidax-fee shadow-run simulation above, and **not** Section 10's Quidax execution path. The user trades BTCUSD manually via MetaTrader 5 through Exness (a CFD venue, not spot) and wanted the same strategy logic to produce advisory entry/exit recommendations for that discretionary trading, reviewed and acted on by hand — the bot never places an order anywhere, on any venue, in this layer either.

- `python main.py recommend`: one process runs every strategy listed in `config.yaml`'s `recommend.strategies` (defaults to the same five configs the shadow runs use, extensible by adding a line to that list — no code change). One shared candle backfill/Binance poll per cycle rather than one per strategy, since the whole point was resource-sharing, not duplicating the five-container shadow-run architecture for a second purpose.
- Reuses, unchanged: `Strategy`/`diagnose()`, the Binance BTC/USDT candle data already backfilled by the shadow containers, `bot/backtest/engine.py`'s `open_position`/`check_exit`/`close_position` (fee/slippage swapped for Exness CFD-cost placeholders, not Quidax's spot fee — untested, tune to your actual account), and the shadow runner's own `drop_incomplete_bar`/`maybe_send_near_miss_alert`/`maybe_send_daily_summary`/`maybe_send_heartbeat` helpers directly. A recommendation's "currently open, hypothetically-recommended" position is tracked via the exact same `paper_position`/`paper_trades`/`signals` tables and functions the shadow runs use, just under a `reco_`-prefixed `strategy_label` (e.g. `reco_donchian_adx_control`) so it can never be confused with a real shadow-run paper trade.
- New alert types: `RECOMMENDATION_ENTRY` (merges in `Signal.context` — regime, ATR%, funding rate, etc. — as the "why," plus a Fear & Greed reading), `RECOMMENDATION_EXIT`, and `RECOMMENDATION_STOP_UPDATE` — the shadow runner updates a trailing stop silently; for a manually-managed MT5 position the moved stop has to actually reach the user or their real stop order goes stale.
- **Alert lifecycle per recommendation**: exactly one `RECOMMENDATION_ENTRY` (always carries `entry=`/`stop=`/`target=` — the stop is the initial invalidation level), zero or more `RECOMMENDATION_STOP_UPDATE`s while it's open (trailing-stop strategies only), then exactly one `RECOMMENDATION_EXIT` (`reason=stop`/`take_profit`/`end_of_data`) closing it out — so a past entry alert is never ambiguous about whether it's still "in play." Current state is also visible on demand via a strategy's daily summary `position=` field (`flat` vs `long since ...`).
- **Reading the daily summary's `diag_*` fields**: `diag_near_miss=False` always pairs with a blank `diag_near_miss_reason=` — the reason is `None` whenever there's no near-miss to explain, and the formatter renders `None` as an empty string rather than omitting the field. Not a bug; the reason only carries real text once `diag_near_miss=True`.
- **Market-wide sentiment context**: `bot/data/sentiment.py` fetches the Crypto Fear & Greed Index (`alternative.me`, public/free/no key), refreshed at most once/day (it only updates that often), stored in a `fear_greed_index` table. Advisory context only, attached to recommendation alerts — deliberately **not** wired into any strategy's entry/exit rule; that would be a new, separately-backtested strategy variant, not "just report it."
- Explicitly out of scope for now: a live Exness/MT5 data feed (signals stay sourced from Binance spot candles — a flagged spot-vs-CFD approximation, not claimed identical) and a live Telegram↔Claude sentiment bridge (the user asked, then asked to pause on it — noted here as a real, separate future feature, not built).
...
4. **Phase 4 — Shadow run**: let it run live on the VPS in paper/alert-only mode for a few weeks, compare real-time behavior against backtest expectations before trusting it. **Acceptance bar**: grade this against "holds up near breakeven while meaningfully cutting drawdown and time-in-market versus holding," not "beats buy-and-hold" — everything tested so far points to an edge somewhere between roughly zero and modest, and grading against outperformance sets up a comparison the strategy was never shown to support.
   - **Concurrent shadow runs (2026-09-07)**: rather than testing one candidate at a time and switching linearly once each finishes gathering data, the shadow-run infrastructure supports multiple strategies running concurrently against the same candle data, each with fully independent paper-trading state (`signals`/`paper_position`/`paper_trades` all keyed by `strategy_label`, not shared capital). `python main.py shadow --strategy <name> --strategy-label <label>` runs any registered strategy standalone; `docker-compose.yml` runs the deployed control (`donchian_adx_control`) alongside four candidates — `multi_timeframe`, `donchian_natr_regime`, `vol_expansion`, `donchian_funding_filtered` — as separate services sharing one SQLite file (WAL mode) and one Telegram chat (every alert is tagged `strategy=<label>`). New hypotheses can be added as another compose service without touching the others.
